...
public class ScrollNumber extends View {
    ...
    private Runnable mScrollRunnable = new Runnable() {
        @Override
        public void run() {
//            float x = (float) (1.0 * (mTargetNum - mCurNum) / mDeltaNum);
            mOffset -= 0.01f;
//            if(mInterpolator.getInterpolation(x)!=0){
//                mOffset *= mInterpolator.getInterpolation(x);
//            }
//            mOffset -= mVelocity * 0.01f * (mInterpolator.getInterpolation(x) );
            invalidate();
    ...
    private void drawText(Canvas canvas) {
        int y = getMeasuredHeight() / 2;
        canvas.drawText(mText, mTextCenterX, y + mTextHeight / 2, mPaint);
    }
